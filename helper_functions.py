...
def extract_coordinates(point_str):
    ...
    Parameters:
    - point_str (str): A string in the format 'POINT (longitude latitude)'
        or 'NA'/None for missing values.

    Returns:
    - tuple: A tuple (longitude, latitude) as floats, or (None, None)
        if the input is 'NA' or missing.
    """
...
def process_hhs_data(data):
    ...
    data['state'] = data['state'].\
        apply(lambda x: x if re.match(r'^[a-zA-Z]{2}$', str(x)) else None)

    # Replace 'NA' values in categorical columns with None
    categorical_columns = \
        ['hospital_name', 'address', 'city', 'zip', 'fips_code']
...
def process_cms_data(data):
    ...
    - The function performs the following transformations:
      ...
      - Filters for valid hospital primary keys (6 characters).
      - Converts 'emergency_services' to a boolean
